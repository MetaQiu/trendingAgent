...
export function DateSelector({ dates, currentDate }: { dates: string[]; currentDate?: string }) {
  ...
  return (
    <div className="lp-card flex flex-wrap items-center gap-2 p-4">
      ...
      <ArrowButton direction="prev" href={olderDate ? `/trending/${olderDate}` : undefined} disabled={!olderDate} />
      <Link className="inline-flex items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--bg-elev)] px-4 py-2 font-mono text-sm font-semibold lp-ink" href={`/trending/${selectedDate}`}>
      ...
      <ArrowButton direction="next" href={newerDate ? `/trending/${newerDate}` : undefined} disabled={!newerDate} />
      {isLatest ? (
        <span className="lp-chip lp-chip-active px-5 py-2 font-semibold">Latest</span>
      ) : (
        <Link className="lp-chip lp-chip-active px-5 py-2 font-semibold" href={`/trending/${latestDate}`}>Latest</Link>
      )}
    </div>
  );
}
